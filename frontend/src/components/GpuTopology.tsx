  ...
  return (
    <section className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <h2 className="font-semibold text-slate-200 text-sm flex items-center gap-2">
            <Gauge className="w-4 h-4 text-indigo-400" /> GPU 实时拓扑与显存
          </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
        {gpus.map((gpu) => (
          <div key={gpu.id} className="bg-slate-950/80 border border-slate-800/80 rounded-lg p-3 relative overflow-hidden">
            <div className="flex items-center justify-between text-xs mb-1.5 font-mono">
              <span className="font-bold text-slate-300">GPU {gpu.id}</span>
              <span className="text-slate-400">{gpu.temp}°C · {gpu.power}W</span>
            </div>
            {/* 显存进度条 */}
            ...
            <div className="flex items-center justify-between text-[11px] text-slate-400 font-mono">
              ...
              <span className={gpu.memPct > 50 ? 'text-indigo-400 font-semibold' : 'text-slate-500'}>
                {gpu.memPct}%
              </span>
            </div>
          </div>
